...
def parse_args():
  ...
  p.add_argument('--data', default="data/sample_json_train.txt", type=str, help='[in] source data')
...
def pre_process(data):
  ...
  入力データを以下と仮定
  #技id
  {accelX:127,accelY:127,accelZ:127,gyroX:187,gyroY:106,gyroZ:101,pressure:101,bendThumb:130,bendIndex:99,bendMiddle:1,bendRing:66,bendPinkie:144}  
  """
  ans = ""
  ...
  for line in open(data):
    ...
    if "accelX" in logs and len(logs["accelX"]) == 3:
      ...
      log_lists.append([log_list, int(ans)])
      logs = defaultdict(lambda: [])
...
def train(args):
  ...
  log_trains = log_lists[:int(0.8*len_data)]
  log_tests = log_lists[int(0.8*len_data):]  
